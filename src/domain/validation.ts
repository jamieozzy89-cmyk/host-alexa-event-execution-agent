...
function assertNonEmpty(value: string, field: string): void {
  if (value.trim().length === 0) throw new DomainError(`${field} must not be empty.`, "INVALID_INPUT");
}

function assertPositive(value: number, field: string): void {
  if (!Number.isFinite(value) || value <= 0) throw new DomainError(`${field} must be a positive finite number.`, "INVALID_INPUT");
}

function assertNonNegative(value: number, field: string): void {
  if (!Number.isFinite(value) || value < 0) throw new DomainError(`${field} must be a non-negative finite number.`, "INVALID_INPUT");
}
...
export function validateEventCreateInput(input: EventCreateInput): void {
  ...
  if (Number.isNaN(Date.parse(input.startAt))) throw new DomainError("event.startAt must be a valid date/time string.", "INVALID_INPUT");
...
export function validateMenu(menu: Menu): void {
  ...
  if (menu.items.length === 0) throw new DomainError("menu.items must contain at least one item.", "INVALID_INPUT");
  ...
  for (const item of menu.items) {
    ...
    for (const template of item.taskTemplates) {
      ...
      if (taskIds.has(template.id)) throw new DomainError(`Duplicate task template id: ${template.id}.`, "DUPLICATE_TASK_ID");
  ...
  for (const item of menu.items) {
    for (const template of item.taskTemplates) {
      for (const dependency of template.dependencies) {
        if (!taskIds.has(dependency)) throw new DomainError(`Task ${template.id} depends on unknown task ${dependency}.`, "UNKNOWN_TASK_DEPENDENCY");
      }
    }
  }
  const dependencyMap = new Map<string, string[]>();
  for (const item of menu.items) for (const template of item.taskTemplates) dependencyMap.set(template.id, template.dependencies);
  const visiting = new Set<string>();
  const visited = new Set<string>();
  const visit = (taskId: string): void => {
    if (visiting.has(taskId)) throw new DomainError(`Preparation dependency cycle detected at ${taskId}.`, "TASK_DEPENDENCY_CYCLE");
...
export function assertMenuSatisfiesConstraints(menu: Menu, constraints: Constraint[]): void {
  ...
  for (const constraint of confirmed) {
    if (constraint.type === "allergen") {
      ...
      if (violating.length > 0) throw new DomainError(`Menu ${menu.id} does not satisfy allergen constraint ${constraint.id}; missing ${requiredTag} on: ${violating.map((item) => item.id).join(", ")}.`, "MENU_CONSTRAINT_CONFLICT");
    }
    if (constraint.type === "dietary") {
      ...
      if (value.includes("vegan")) {
        ...
        if (!hasSuitableItem) throw new DomainError(`Menu ${menu.id} does not provide a vegan-compatible item required by ${constraint.id}.`, "MENU_CONSTRAINT_CONFLICT");
        ...
        if (!hasSuitableItem) throw new DomainError(`Menu ${menu.id} does not provide a vegetarian-compatible item required by ${constraint.id}.`, "MENU_CONSTRAINT_CONFLICT");
      }
    }
  }
}
